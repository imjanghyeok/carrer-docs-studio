  ...
  async function insert() {
    ...
    try {
      ...
      const svg = await exportToSvg({
        elements,
        appState: { ...appState, exportWithDarkMode: false, exportBackground: false },
        files,
        exportPadding: 20,
      });
      svg.style.width = '100%';
  ...
  return (
    <>
      <header className="diagram-header">
        <div>
          <b>다이어그램 작업실</b>
          <span>도형 · 화살표 · 텍스트 · 자유 그리기</span>
        </div>
        <div>
          <button
            onClick={() => {
              if (
                api.current?.getSceneElements().length &&
                !window.confirm('문서에 반영하지 않고 닫을까요?')
              )
                return;
              onClose();
            }}
      ...
      <div className="excalidraw-container">
        <Excalidraw
          ...
          initialData={
            initial || { appState: { viewBackgroundColor: '#ffffff', currentItemFontFamily: 2 } }
          }
          langCode="ko-KR"
          theme="light"
          UIOptions={{
      ...
      <footer className="diagram-footer">
        도형 데이터도 함께 저장되어 나중에 다시 편집할 수 있습니다.
      </footer>
    </>
  );
}
