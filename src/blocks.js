...
export function textBlock(node) {
  ...
  if (
    !el ||
    el.closest('figure,svg,table,.activity-chart') ||
    el.querySelector('p,div,ul,ol,table,figure,svg,h1,h2,h3,blockquote')
  )
    return null;
  return el;
}
export function blockKind(node) {
  const el = textBlock(node);
  if (!el) return '';
  return el.matches('li')
    ...
    : el.matches('p')
      ? 'paragraph'
      : el.matches('blockquote')
        ? 'quote'
        : el.tagName.toLowerCase();
}
function resetIds(node) {
  for (const el of [node, ...node.querySelectorAll('*')]) {
    el.removeAttribute('id');
    delete el.dataset.studioId;
    delete el.dataset.studioSelected;
  }
}
function focus(editor, el) {
  editor.instrument();
  editor.select(el, true);
  const target = el.matches('ul,ol') ? el.firstElementChild : el;
  target?.focus();
}
function make(doc, kind) {
  const tag =
    { paragraph: 'p', bullet: 'ul', numbered: 'ol', quote: 'blockquote', divider: 'hr' }[kind] ||
    kind;
...
function replaceItem(item, replacement) {
  ...
  if (after.children.length) replacement.after(after);
  item.remove();
...
export function insertBlock(editor, kind) {
  ...
  editor.mutate(() => {
    const node = make(editor.doc, kind),
      text = textBlock(editor.selected),
      target = text || editor.selected;
    if (target?.matches('li')) {
      const list = target.parentElement;
      if (
        (kind === 'bullet' && list.tagName === 'UL') ||
        (kind === 'numbered' && list.tagName === 'OL')
      ) {
      ...
      const marker = editor.doc.createElement('li');
      target.after(marker);
      replaceItem(marker, node);
    } else if (target?.matches('.page,main,section,article,div,header,footer,aside'))
      target.append(node);
...
export function convertBlock(editor, kind) {
  const old = textBlock(editor.selected);
  if (!old || !blockKinds[kind] || kind === 'divider') return false;
  if (blockKind(old) === kind) return true;
  editor.mutate(() => {
    const replacement = make(editor.doc, kind),
      target = replacement.matches('ul,ol') ? replacement.firstElementChild : replacement;
    for (const attr of old.attributes)
      if (!['contenteditable', 'spellcheck', 'data-studio-selected'].includes(attr.name))
        target.setAttribute(attr.name, attr.value);
    while (old.firstChild) target.append(old.firstChild);
    if (old.matches('li')) replaceItem(old, replacement);
    else old.replaceWith(replacement);
    focus(editor, target);
  });
  return true;
}
export function removeBlock(editor) {
  const el =
    textBlock(editor.selected) || (editor.selected?.matches('hr') ? editor.selected : null);
  if (!el) return false;
  editor.mutate(() => {
    const next = el.nextElementSibling || el.previousElementSibling,
      parent = el.parentElement;
    el.remove();
    if (parent.matches('ul,ol') && !parent.children.length) parent.remove();
    editor.selected = null;
    editor.onSelect(null);
    if (next?.isConnected) editor.select(next, true);
  });
  return true;
}
export function splitBlock(editor, event) {
  const range = editor.doc.getSelection()?.rangeCount
    ? editor.doc.getSelection().getRangeAt(0)
    : null;
  const old = textBlock(event.target);
  if (!old || !range || !old.contains(range.startContainer) || !old.contains(range.endContainer))
    return false;
  event.preventDefault();
  editor.selected = old;
  if (old.matches('li') && !old.textContent.trim()) {
    convertBlock(editor, 'paragraph');
    return true;
  }
  editor.mutate(() => {
    range.deleteContents();
    const tail = editor.doc.createRange();
    tail.selectNodeContents(old);
    tail.setStart(range.startContainer, range.startOffset);
    const node = editor.doc.createElement(old.matches('li') ? 'li' : 'p');
    node.append(tail.extractContents());
    resetIds(node);
    old.after(node);
    focus(editor, node);
    const caret = editor.doc.createRange();
