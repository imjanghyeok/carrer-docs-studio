...
test('platform data defaults are separate from the checkout',()=>{
 ...
 assert.equal(defaultDataDirectory({},'darwin',home),path.join(home,'Library','Application Support','Career Studio'));
 assert.equal(defaultDataDirectory({},'linux',home),path.join(home,'.local','share','career-studio'));
 assert.equal(defaultDataDirectory({},'win32',home),path.join(home,'AppData','Local','Career Studio'));
...
test('asset resolution rejects traversal and symlink escapes',async()=>{
 ...
 const inside=path.join(dir,'assets');await fs.mkdir(inside);await fs.writeFile(path.join(inside,'ok.css'),'body{}');
