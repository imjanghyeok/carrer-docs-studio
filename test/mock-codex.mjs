...
createInterface({ input: process.stdin }).on('line', (line) => {
  ...
  if (method === 'thread/start') {
    assert.equal(p.permissions, 'career_studio');
    assert.equal(p.approvalPolicy, 'never');
    assert.equal(p.ephemeral, true);
    assert.equal(p.config['mcp_servers.test_server.enabled'], false);
  ...
  if (method === 'turn/start') {
    ...
    const result = {
      message: input.mode.startsWith('Answer')
        ? '샘플 문구에 대한 답변입니다.'
        : '요청한 샘플 문구를 다듬었습니다.',
      edits: input.mode.startsWith('Answer')
        ? []
        : [
            {
              before: input.editableHtml,
              after: input.editableHtml.replace(/테스트 전|테스트 후/g, '테스트 후'),
              reason: '샘플 문구 변경',
            },
          ],
    };
    if (input.request === 'css')
      result.edits = [
        {
          before: 'h1{font-size:32px}',
          after: 'h1{font-size:32px;color:rgb(1,2,3)}',
          reason: '문서 스타일 변경',
        },
      ];
      ...
      setTimeout(
        () => {
          ...
          send({
            ...
            params: { threadId: p.threadId, turn: { id: turnId, status: 'completed' } },
          });
          timers.delete(p.threadId);
        },
        cancel ? 30000 : 150,
      ),
    );
