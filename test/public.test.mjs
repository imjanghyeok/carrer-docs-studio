...
test('local planning notes are excluded but force-added notes block publication', async () => {
  ...
  await fs.copyFile(
    new URL('../scripts/check-public.mjs', import.meta.url),
    path.join(root, 'scripts/check-public.mjs'),
  );
  await fs.writeFile(
    path.join(root, 'PUBLIC_FILES.json'),
    JSON.stringify(['PUBLIC_FILES.json', '.gitignore', 'scripts/check-public.mjs']),
  );
  await fs.writeFile(path.join(root, '.gitignore'), '.local/\n');
  await fs.writeFile(path.join(root, '.local/issue.md'), 'Synthetic local draft');
  const run = () =>
    spawnSync(process.execPath, ['scripts/check-public.mjs'], { cwd: root, encoding: 'utf8' });
  execFileSync('git', ['init', '-q'], { cwd: root });
  assert.equal(run().status, 0);
  execFileSync('git', ['add', '-f', '.local/issue.md'], { cwd: root });
  const blocked = run();
  assert.equal(blocked.status, 1);
  assert.match(blocked.stderr, /tracked outside public allowlist/);
